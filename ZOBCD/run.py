...
class OurArguments(TrainingArguments):
    ...
    project: str = "opt-1-3b-sst2"  # wandb project name
    # selective
    selective: bool = False  # whether to use selective
    selective_ratio: float = 0.5  # selective rate
    adaSelect: bool = False  # whether to use ada_select
    # block coordinates descent
    bcd: bool = False  # whether to use block coordinates descent under selective condition
    lora_bcd: bool = False  # whether to use block coordinates descent for LoRA
    # seed
    ...
    tag: str = ""  # saving tag
    wandb_name: str = "test"  # wandb name
...
class Framework:
    ...
    def load_model(self):
        ...
        """
        local_path = self.args.model_name
        with count_time(f"Loading model with {'FP16' if self.args.load_float16 else ('bfloat16' if self.args.load_bfloat16 else 'FP32')}"):
            ...
            if self.args.no_auto_device:
                ...
                # model = AutoModelForCausalLM.from_pretrained(local_path, config=config, device_map='auto',
                ...
                model = AutoModelForCausalLM.from_pretrained(local_path, torch_dtype=torch_dtype)
        ...
        if self.args.lora:
            if self.args.selective:
                # Selective LoRA
                from lora_ms import LoRA
                LoRA(model, r=self.args.lora_r, alpha=self.args.lora_alpha, bcd=self.args.bcd,
                              args=self.args)
            else:
...
def main():
    ...
    wandb.init(project=args.project, name=args.wandb_name, config=args)
